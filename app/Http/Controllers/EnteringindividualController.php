...
class EnteringindividualController extends Controller
{
    ...
    public  function store(Request $request)
    {
        ...
        // $id_ef = Enteringpeaple::where('code_melli', $request->input('code_melli'))->orderBy('id_ep', 'DESC')->get()->first()->id_ef;
        // $company = Enteringform::where('id_ef', $id_ef)->orderBy('id_ef', 'DESC')->get()->first()->company;
        // Enteringindividual::where('code_melli', $request->input('code_melli'))->update(['company'=>$company]);

        Enteringpeaple::where('code_melli', $request->input('code_melli'))->update(['presence'=>1]);
...
//        $data = Enteringindividual::where('code_melli','=',$id)->orderBy('i_ed', 'DESC')->limit(2)->get()->toArray();
    ...
    public function total_individuals($date1,$date2)
    {
        ...
        $forms = EnteringForm::where('id_ef','>',0)->orderBy('id_ef', 'ASC')->get()->toArray();
        ...
        return response()->json(['success'=>'hi','results'=> $individuals,'peaples'=>$peaples,'users'=>$users,'x'=>$date1,'y'=>$date2,'forms'=>$forms]);
        // ,'forms'=>$forms
    }

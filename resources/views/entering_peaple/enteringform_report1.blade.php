                    ...
                    success: function (response) {
                        ...
                        for(var i = 0; i < response.results.length; i++) {
                           link='./selectcar/'+response.results[i]['id_ec']
                            ...
                            if(response.results[i]['car_no'] != null){
                                ...
                                car_no1 = $('<td style="width: 3%">' + '-' +'</td>')
                                car_no2 = $('<td style="width: 2%">' + '-' +'</td>')
                                car_no3 = $('<td style="width: 3%">' + '-' +'</td>')
                            }
            ...
            $('#third2_report').click(function(event) {
                ...
                $.ajax({
                    ...
                    success: function (response) {                            
                        ...
                        for(var i = 0; i < response.result.length; i++) {
                            id_b=$('<td style="display: none">' + response.result[i]['id_b'] + '</td>')
                            f_name = $('<td style="width: 10%;text-align:center;font-size:10px">' + response.result[i]['f_name'] + '</td>')
                            l_name = $('<td style="width: 10%;text-align:center;font-size:10px">' + response.result[i]['l_name'] + '</td>')
                            national_code = $('<td style="width: 10%;text-align:center;font-size:10px">' + response.result[i]['national_code'] + '</td>')
                            company_name = $('<td style="width: 20%;text-align:center;font-size:10px">' + response.result[i]['company_name'] + '</td>')
                            reason = $('<td style="width: 40%;text-align:center;font-size:10px">' + response.result[i]['reason'] + '</td>')
       ...
       <div class="modal-dialog modal-md" id="editlist2" style="margin-top: 50px;margin-left: 460px">
        <div class="modal-content">
            ...
            <div class="modal-footer bg-info" style="height: 20px;width:600px">
                    ...
                    <div class="toast bg-success individuals333" style="margin-top:-15px;margin: auto;border-radius: 10px">
                ...
                <div class="row" style="margin-top:10px">
                    ...
                    <div id="person_div88_edit" class="col" style="height:50px">
                        ...
                        <div id="s2_edit" class="container" style="text-align: left;background-color:#17a2b8;width: 100%;border-radius: 5px;height:125px;direction: rtl;color: white;margin-top:2px;padding-top: 2px;">
                            <form method="post" encType="multipart/form-data" id="addindividuals2" action="{{route('updateindividuals.store')}}">
                                {{csrf_field()}}
                                <div class="row" style="height: 15px">
                                    <div class="col-9">
                                ...
                                <br>
                                <br>
                                <input hidden type="text" id="code_melli_edit" name="code_melli">
